...
async def mute_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_action(context, f"🔇 Mute\nKullanıcı: {target.full_name} ({minutes} dk)")
...
# ====================== MÜZİK (Şarkı adı ile) ======================
async def muzik_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("❌ Kullanım: `/müzik Şarkı Adı Sanatçı`", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    ydl_opts = {
        ...
        'noplaylist': True,
        'quiet': True,
        'default_search': 'ytsearch1',   # Şarkı adı ile arama
    ...
    try:
        ...
        await msg.edit_text(f"❌ İndirme hatası: {str(e)[:200]}")
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⚠️ Warn", callback_data="panel_warn"), InlineKeyboardButton("🔄 Unwarn", callback_data="panel_unwarn")],
...
def main():
    ...
    app.add_handler(CommandHandler("müzik", muzik_cmd))

    print("🚀 Güvenlik Botu + Şarkı Adı ile Müzik aktif!")
    app.run_polling()
